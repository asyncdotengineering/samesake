...
export function d1EnrichStore(db: DB, candidates?: DedupCandidateProvider): EnrichStore {
  ...
  return {
    async upsert(rows) {
      ...
      const update = db.prepare(
        `UPDATE catalog SET data = ?, content_hash = ?, pipeline_status = 'pending',
         ...
         gate_reason = NULL, doc = NULL, vectors = NULL, rerank_doc = NULL, fts_src = NULL,
    ...
    async writeEnriched(rows: EnrichedRow[]) {
      const update = db.prepare(
        `UPDATE catalog SET enriched = ?, pipeline_status = ?, gate_reason = ?,
         doc = ?, vectors = ?, rerank_doc = ?, fts_src = ?, fts_src_a = ?, attempt_count = 0,
      ...
      for (const row of rows) {
        update.run(
          ...
          row.vectors ? JSON.stringify(row.vectors) : null,
    ...
    async loadEnriched(limit) {
      const rows = db.prepare(
        `SELECT id, enriched, vectors, doc, rerank_doc, fts_src, fts_src_a, pipeline_status, gate_reason
      ...
      return rows.map((row) => ({
        ...
        vectors: row.vectors ? JSON.parse(row.vectors) as Record<string, number[]> : undefined,
        surfaces: {
          doc: row.doc,
          denseByEmbedding: {},

...
export const products = collection("products", {
  ...
  // An enrich pipeline runs before indexing. This catalog's data is already clean,
  // so the pipeline is empty and the surfaces below read the raw `data` directly. A
  // messier catalog adds stages here to parse structured attributes into `enriched` first.
  enrich: { stages: [] },
  indexing: {
    surfaces: {
      embed_doc: {
  ...
  search: {
    ...
    // it, the query text is treated as keywords only and budgets are never parsed.
...
<Aside type="tip" title="Run this end to end">
The complete, runnable version of this guide — the exact `catalog.ts` and `search.ts` above, plus a test that proves the budget exclusion against a real Postgres — lives in [`examples/build-a-search-experience`](https://github.com/asyncdotengineering/samesake/tree/main/examples/build-a-search-experience). `SAMESAKE_DATABASE_URL=… bun run src/run.ts`.
</Aside>

...
3. **Migrate, push, enrich.**
   ...
   <Aside type="tip">
   `enrich.enrich()` builds the FTS + dense surfaces, embeds the dense ones through your
   `embed` function, and writes the vectors back — so both the `fts` and `cosine` channels
   are live. With the stub embed above it runs keyword-only; swap in a real model for meaning.
   </Aside>

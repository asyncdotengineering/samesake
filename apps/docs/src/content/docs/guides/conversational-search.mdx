...
<Aside type="caution">
The floor is an **absolute** threshold, so it's specific to your embedding model and drifts with the corpus. `0.5` is calibrated for `gemini-embedding-2`; recalibrate with the [eval gate](/guides/eval-gate/) if you change models. Diagnose any surprising empty/padded result with the search result's `parsed`/`constraintTrace` first.
</Aside>
...
## Inspect everything on the result

When a result surprises you — too many, too few, the wrong order — don't guess. Every `search()`
call returns the parse and the applied constraints:

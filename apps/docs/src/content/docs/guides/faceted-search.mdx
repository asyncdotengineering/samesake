...
`facet: "range"` on a number field gives you histogram buckets (min, max, and a handful of lo/hi bands). Everything else with `facet: true` returns value → count pairs.

...
<Aside type="tip">
The Tier-2 `SamesakeBundle` returned by `samesake()` exposes `facets()` alongside
`enrich`, `resolve`, and `search`, so query-free aggregations can use the same bundle.
</Aside>
...
Copy the provider adapters from [Providers](/reference/providers/), `migrate` once, then the `enrich.upsert` → `enrich.enrich` loop above.

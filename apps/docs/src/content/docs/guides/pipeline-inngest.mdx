...
[Inngest](https://www.inngest.com/) turns each bundle call into a **durable step**. If `enrich` fails, Inngest retries just that step — `upsert` doesn't re-run. Cron triggers give you the safety-net loop; events give you webhook fast-paths. You serve the functions from your existing Node app.

See [Running the enrich pipeline durably](/guides/enrich-pipeline/) for the model. This page wires it to Inngest.

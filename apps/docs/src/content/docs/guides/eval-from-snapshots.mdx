...
Push the same products through the real pipeline — `enrich.upsert` → `enrich.enrich` — into a
dedicated schema/table, then run the same queries through `app.search` (NLQ + FTS + cosine ANN).

<Aside type="caution" title="Use a dedicated schema">
A reused `schema`/`table` can carry an *orphaned* catalog from a prior run — there is no project
registry to clean up after in the new bundle, just the Postgres tables `migrate()` created. Pass a
fresh `schema` (or `table`) per eval run and check the **table row count** directly for an isolated,
deterministic eval.
</Aside>
...
For graded relevance (nDCG@K, Hit@K, MRR) over the same golden set, use `matcher.runEval` (still a
`@samesake/server` `Matcher`-only capability — see [Eval gate](/guides/eval-gate/) for the gap) via

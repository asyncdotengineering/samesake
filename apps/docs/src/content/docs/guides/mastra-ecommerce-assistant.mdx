...
`@samesake/server` ships no AI SDK — you pick the models. This app uses the ready-made Gemini
adapters from `@samesake/providers` (retry/backoff and bulk-indexing throttling built in):
...
export function getMatcher() {
  ...
  _matcher = createMatcher({
    ...
    migrate: "eager",
    embed: geminiEmbedder({ minIntervalMs: 120, retries: 8 }),

...
<Aside type="tip" title="samesake.search() needs a query — aggregations don't go through it">
`search()` requires a non-empty `q` (or image) — it's a retrieval call, not a browse-and-group call, and it can't express a `GROUP BY`. So *"which brand lists the most shoes"* and *"average price for brand X"* run as **direct SQL** against the compiled collection table (`project_qa_ecommerce.c_products`), exactly like the recipe's agent running an aggregation in the database.
</Aside>

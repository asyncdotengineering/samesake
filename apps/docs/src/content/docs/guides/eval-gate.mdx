...
<Aside type="caution">
`runEval` is a method on the legacy `@samesake/server` `Matcher` instance — it has no
standalone export and has not been ported to `@samesake/query`/`@samesake/postgres`. The example
below still goes through `createMatcher` for the orchestration (query loop, judge-family assertion,
artifact + judge-cache writes); only the underlying search index it evaluates needs to be the same
Postgres data your `samesake()` bundle indexed. Composing this loop by hand against `app.search`
plus the standalone `makeLlmJudge` + metric functions (`ndcgAtK`, `hitAtK`, `mrr`, `nullRate`,
`constraintViolations`, all real `@samesake/server` exports) is possible but nontrivial — do that only
if you need to drop the `@samesake/server` dependency entirely.
</Aside>
...
- [Tuning search relevance](/guides/tuning-search/) — diagnose with the search result's `constraintTrace`, fix data, then measure.

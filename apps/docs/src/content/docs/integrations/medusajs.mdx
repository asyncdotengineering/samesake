...
export async function bulkIndex(container, app: SamesakeBundle) {
...
export default async function handler({ event: { data }, container }: SubscriberArgs<{ id: string }>) {
  const query = container.resolve("query");
  const app = container.resolve("samesake"); // your module wrapping samesake()
  ...
  if (p?.status === "published") {
    await app.enrich.upsert([toDoc(p)]);
    await app.enrich.enrich();
  } else {
    await app.enrich.remove([data.id]);
  }
}
...
A second subscriber on `product.deleted` runs the same soft-delete upsert — see the TODO
above; there is no hard-delete port on the current Tier-2 bundle.
...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  const app = req.scope.resolve("samesake");
  const { q, filters, limit } = req.body as any;
  res.json(await app.search(q, { filters, limit: limit ?? 20 }));
}
```
...
1. Custom **samesake module** (`src/modules/samesake`) wrapping `samesake()` from `@samesake/postgres`.
2. **Bulk index** via the Query graph on first boot / a one-off admin route.
3. **Subscribers** for `product.created|updated|deleted` → upsert/enrich or soft-delete.
4. **`/store/products/search`** proxying to `app.search`.

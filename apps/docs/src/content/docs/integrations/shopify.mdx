  ...
  indexing: {
    surfaces: {
      embed_doc: {
        ...
        embedding: "doc",
        build: ({ data }) =>
          `${data.title} ${data.description} ${data.product_type} ${data.vendor}`.trim(),
      },
      fts_doc: {
        kind: "fts",
        build: ({ data }) =>
...
3. Reassemble: nested connections (variants, images) are separate lines carrying `__parentId`. Group them onto the parent, map to docs, and `enrich.upsert` in batches, then `enrich.enrich` once.
...
if (topic === "products/delete") {
  await app.enrich.remove([String(product.id)]);
} else {
  await app.enrich.upsert([toDoc(product)]);
  await app.enrich.enrich();
}
```
...
2. **Initial index:** one Admin GraphQL bulk operation → stream JSONL → batch `enrich.upsert` → `enrich.enrich`.
3. **Deltas:** `products/create|update|delete` webhooks → verify HMAC → upsert/enrich or soft-delete.

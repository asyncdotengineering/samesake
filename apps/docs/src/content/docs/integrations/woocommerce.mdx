...
async function bulkIndex(base: string, auth: string, app: SamesakeBundle) {
  ...
  do {
    ...
    await app.enrich.upsert(products.map(toDoc));
    page++;
...
if (topic === "product.deleted") {
  await app.enrich.remove([String(payload.id)]);
} else {
  await app.enrich.upsert([toDoc(payload)]);
  await app.enrich.enrich();
}
```
...
3. **Webhooks:** `product.created|updated|deleted` → verify raw-body HMAC → upsert/enrich or soft-delete.

...
Wire `rerank` on `samesake({ rerank: myRerank })` (the Tier-2 bundle) or directly on
...
`llmRerank` wraps [`makeLlmJudge`](/reference/relevance-judge/) and maps the ESCI gain `{0..3} → score/3`.

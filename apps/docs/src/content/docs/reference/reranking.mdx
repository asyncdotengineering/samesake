---
title: Reranking
description: Second-stage reranking in samesake — position-aware blend with first-stage RRF, the RerankFn contract, llmRerank, and remote adapter examples.
---
...
`llmRerank` wraps [`makeLlmJudge`](/reference/relevance-judge/) and maps the ESCI gain `{0..3} → score/3`. It is **not** bundled or auto-on — wire it explicitly:
...
- [Relevance judge](/reference/relevance-judge/) — the LLM judge behind `llmRerank` and `runEval`

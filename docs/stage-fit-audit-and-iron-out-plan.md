...
| **Typed spaces (`space_vec`)** | **SUPERSEDED 2026-07-18 — delete with the multi-aspect major** | V02g failed its gate; off-default since. The multi-aspect RFC (`rfcs/rfc-multi-aspect-retrieval.md` §0, approved full-scope) carries the typed-multi-signal story with per-aspect columns + query-aware routing; structural segments (price/freshness/category) migrate to filters/`RankingPolicy`/recency where they belong. Deleted in the same major once image-query parity holds (REQ-10/C10). |

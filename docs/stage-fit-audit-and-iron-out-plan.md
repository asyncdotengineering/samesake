...
| **`ts_rank_cd` lexical leg, configurable language** | **RESOLVED 2026-07-18 — challenge closed, BM25 dropped** | Free wins (`setweight`, normalise/phonetic, configurable FTS language) shipped in P1; the BM25 extension bake-off is dropped — incumbent product-search engines don't use BM25, and no viable extension fits deployment targets. Lexical quality is guarded by the A/B fixture. |
...
| **Typed spaces (`space_vec`)** | **SUPERSEDED 2026-07-18 — delete with the multi-aspect major** | V02g failed its gate; off-default since. The multi-aspect RFC (shipped; removed post-ship, see git history for `rfcs/rfc-multi-aspect-retrieval.md` §0) carries the typed-multi-signal story with per-aspect columns + query-aware routing; structural segments (price/freshness/category) migrate to filters/`RankingPolicy`/recency where they belong. Deleted in the same major once image-query parity holds (REQ-10/C10). |

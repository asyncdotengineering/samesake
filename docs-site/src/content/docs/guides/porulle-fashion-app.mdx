...
enrich: pipeline(
  stage("vision", {
    ...
    schema: () => ({
      type: "object",
      properties: {
        color_text: { type: "string" },          // "light blue and white striped"
...
nlq: { enable: true, semanticRewrite: true,
  schema: { type: "object", properties: { semantic_query: { type: "string" }, max_price: { type: "number" } }, required: ["semantic_query"] } }
```
